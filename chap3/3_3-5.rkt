...
; - (has-value? <connector>)
; - (get-value <connector>)
; - (set-value! <connector> <new-value> <informant>) (  informant :
...
(define (print-constraints connector)
  (connector 'pc))
...
;; multiplier は2つの値があって初めて残り1つの値を定めることができる。
;; 例えば、(a, b) = (?, ?) にしてから (set-value! b 100 ...)を実行しても(a, b) = (?, 100)のまま

(define (bad-squarer a b)
  (multiplier a a b))

(define ba (make-connector))
...
;(set-value! bb 10 'user)
(forget-value! ba 'user)
(set-value! bb 100 'user)
